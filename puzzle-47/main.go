...
func main() {
	...
	lines := readAllLines(workingDirectory + "/puzzle-47/input.txt")
	...
	for indices, values := range pairs {
		modelNumber += int(math.Pow10(indices[0])) * values[len(values)-1][0]
		modelNumber += int(math.Pow10(indices[1])) * values[len(values)-1][1]
	}
	// Verify result
	alu.LoadProgram(lines)
