...
const NumHomes int = 4
const NumPodsPerHome int = 4
...
type State [NumHomes * NumPodsPerHome]Amphipod

func (s State) String() string {
	// TODO: could make this depth dynamic using NumAmphipods
	grid := [][]string{
		{"#", "#", "#", "#", "#", "#", "#", "#", "#", "#", "#", "#", "#"},
		{"#", ".", ".", ".", ".", ".", ".", ".", ".", ".", ".", ".", "#"},
		{"#", "#", "#", ".", "#", ".", "#", ".", "#", ".", "#", "#", "#"},
...
func main() {
	...
	//lines := readAllLines(workingDirectory + "/puzzle-46/test.txt")
	lines := readAllLines(workingDirectory + "/puzzle-46/input.txt")
	...
	for y, line := range lines {
		for x, field := range line {
			if field >= 'A' && field <= 'D' {
				initialState[index] = Amphipod{Vector2D{x, y}, string(field)}
				index++
			}
		}
	}
	...
	// Building graph of state transitions
	graph := Graph{make(map[State]int), make(map[State]map[State]int)}
	graph.SetNode(initialState, 0)
	exploreStateGraph(&graph, initialState)
	fmt.Println("Total of", len(graph.Nodes), "states to consider.")

	// Determining shortest path to target
	target := State{
		{Vector2D{3, 2}, "A"},
		{Vector2D{3, 3}, "A"},
		{Vector2D{3, 4}, "A"},
	...
	predecessors := shortestPath(&graph, map[State]int{initialState: 0})
...
func exploreStateGraph(graph *Graph, state State) {
	...
	// Precompute occupation state of each home
	enterable := map[string]bool{"A": true, "B": true, "C": true, "D": true}
	for i := len(state) - 1; i >= 0; i-- {
		pod := state[i]
		switch pod.Position[0] {
		case 3:
			enterable["A"] = enterable["A"] && "A" == pod.Value
		case 5:
			enterable["B"] = enterable["B"] && "B" == pod.Value
		case 7:
			enterable["C"] = enterable["C"] && "C" == pod.Value
		case 9:
			enterable["D"] = enterable["D"] && "D" == pod.Value
		}
	}
	...
	for index, pod := range state {
		if pod.Position[1] > 1 { // Apply rules for inside any home
			// No moves if home occupants are correct
			if pod.Position[0] == Homes[pod.Value] && enterable[pod.Value] {
				continue
			}
			...
			if !enterable[pod.Value] {
				continue
			}
			var target Vector2D
			xHome := Homes[pod.Value]
			for y := 2; y < 2+NumPodsPerHome; y++ {
				if _, ok := positions[Vector2D{xHome, y}]; !ok {
