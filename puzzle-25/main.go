...
func main() {
	...
	pointToggle := true
	for line := range readLines(workingDirectory + "/puzzle-25/input.txt") {
		if line == "" {
			pointToggle = false
			continue
		}
	...
	for _, instruction := range instructions[:1] {
		if instruction[0] == 0 {
			folded = foldHorizontally(folded, instruction[1])
		}
	}

	totalDots := 0
	for row := range *folded {
		for column := range (*folded)[row] {
			if (*folded)[row][column] > 0 {
				totalDots++
			}
		}
	}
...
func foldHorizontally(grid *[][]int, line int) *[][]int {
