...
func main() {
	...
	origin := Vector{0, 0}
	validInitialVelocities := make(map[Vector]int)
		...
		for vy := targetY[0]; vy <= abs(targetY[0]); vy++ {
			...
			for step := 0; ; step++ {
				...
				if position[0] >= targetX[0] && position[0] <= targetX[1] && position[1] >= targetY[0] && position[1] <= targetY[1] {
					if _, ok := validInitialVelocities[Vector{vx, vy}]; !ok {
						validInitialVelocities[Vector{vx, vy}] = step
					}
				}
	...
	fmt.Println("Number of initial velocities leading to target range hits:", len(validInitialVelocities))
}
